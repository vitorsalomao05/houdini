...
The old issue #5 is still a proposal awaiting retriage. Its full authenticated-use gate is not implicitly adopted by this scope. This document records the owner's accepted implementation boundary. Subsequent production approval and publication evidence belong in `RELEASE.md`; WIPs #3/#4 remain paused.
